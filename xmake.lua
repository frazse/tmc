...
target("tmc_android")
    set_kind("shared")
    set_languages("c11", "cxx20")
    set_targetdir("build/android")

    add_defines("PC_PORT", "NON_MATCHING", "USE_HDMA", "USA", "ENGLISH", "REVISION=0")
    add_defines("TMC_ANDROID_PORT", "TMC_ANDROID_EXPERIMENTAL")
    add_defines('TMC_PC_VERSION="' .. TMC_PC_VERSION .. '"')
    add_defines('TMC_PORT_VERSION="' .. TMC_PC_VERSION .. '"')
    ...
    add_includedirs("libs/ViruaPPU/include")
    add_includedirs("libs/VirtuaAPU/include")
    add_includedirs("libs/agbplay_core")
    add_includedirs("tools/src/assets_extractor")

    -- Android doesn't use the Modern Launcher (submodule missing anyway)
    -- add_defines("launcher", "GUILITE_ON")
    ...
    add_files("tools/src/assets_extractor/assets_extractor_api.cpp")
    add_files("port/port_m4a_backend.cpp")
    add_files("port/generated_sounds_embed.cpp")
    -- add_files("tools/src/assets_extractor/embedded_sounds_json.cpp")
    add_files("port/port_ppu.cpp")
    add_files("port/port_rom.c")
    add_files("port/port_stubs.c")
    add_files("port/stubs_autogen.c")
    add_files("port/data_stubs_autogen.c")
    add_files("port/data_const_stubs.c")
    add_files("port/port_rom_tables.c")
    add_files("port/port_bios.c")
    add_files("port/port_linked_stubs.c")
    add_files("port/port_figurines.c")
    add_files("port/port_draw.c")
    add_files("port/port_gba_mem.c")
    add_files("port/port_hdma.c")
    add_files("port/port_upscale.c")
    add_files("port/port_save.c")
    add_files("port/port_softslots.c")
    add_files("port/port_touch_controls.cpp")
    add_files("port/port_filter.c")
    add_files("port/port_animation.c")
    add_files("port/port_math.c")
    add_files("port/port_text_render.c")
    add_files("port/port_gameplay_stubs.c")
    add_files("port/port_m4a_stubs.c")
    add_files("port/port_room_funcs.c")
    add_files("port/port_script_funcs.c")
    add_files("libs/ViruaPPU/src/*.c")
    add_files("libs/VirtuaAPU/src/*.c")
    add_files("libs/agbplay_core/*.cpp")
    ...
    add_cflags("-Wall", "-Wextra", "-Wno-unused-parameter", "-fno-strict-aliasing", "-fwrapv", "-O2")
    add_cxxflags("-Wall", "-Wextra", "-Wno-unused-parameter", "-fno-strict-aliasing", "-fwrapv", "-O2")
    set_strip("all")
target_end()
